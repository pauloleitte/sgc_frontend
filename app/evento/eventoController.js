(function() {
  ...
  function eventoController($http, msgs, tabs, $location) {
    ...
    vm.listaDeTipos = ["Congresso", "Culto", "Oração"];

    vm.Congregacaos = [{}];

    vm.getCongregacao = function() {
      $http
        .get(`${url_heroku_congregacao}`)
    ...
    vm.addPartipante = function(index) {
      vm.Evento.participantes.splice(index + 1, 0, {
        nome: null,
        sexo: null,
        idade: null
      });
